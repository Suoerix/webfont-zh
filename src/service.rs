use crate::{
    ...
    error::AppError,
    font::FontProcessor,
    utils::{cleanup_expired_cache, generate_cache_filename},
};
use anyhow::Result;
...
impl FontService {
                    ...
                    for font_file in &font_config.files {
                        ...
                        if font_path.exists() {
                            match FontProcessor::new(&font_path) {
                                Ok(processor) => {
                                    let key =
                                        format!("{}:{}", font_config.id, font_file.font_family);
    ...
    pub async fn generate_font(
        ...
        if let Some(id) = font_id {
            ...
            return self
                .generate_font_by_id_internal(id, codepoints, &mut visited)
                .await;
        }
        ...
        for font_id in &font_ids {
            let mut visited = std::collections::HashSet::new();
            if let Ok(woff2_data) = self
                .generate_font_by_id_internal(font_id, codepoints, &mut visited)
                .await
            {
        ...
        // 记录详细的错误信息
        let char_list: Vec<String> = codepoints
            .iter()
    ...
    async fn generate_font_by_id_internal(
        ...
        for fallback_id in &fallback_ids {
            ...
            let fallback_result =
                Box::pin(self.generate_font_by_id_internal(fallback_id, codepoints, visited)).await;
